...
import { LayoutGrid, RefreshCw, Smartphone, AlarmClock, StickyNote, CalendarClock, CheckCircle2, Plus } from 'lucide-react';
import { db } from '../lib/db';
import { buildSnapshot, syncWidgets, requestPinWidget, type WidgetSnapshot } from '../lib/widget';
...
export function WidgetsPage() {
  ...
  const todayEvents = snap?.days[today] ?? [];
  const todayNote = snap?.notes[today] ?? '';
  const nextAlarm = snap?.alarms.length ? [...snap.alarms].sort((a, b) => a.time.localeCompare(b.time))[0] : null;
  async function pin(kind: 'today' | 'note' | 'alarm') { const ok = await requestPinWidget(kind); toast(ok ? 'Choose where to place the widget' : 'Long-press your home screen → Widgets → CaCa', 'info'); }
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className={cn('mb-6 flex items-start gap-3 rounded-2xl border p-4 text-sm', isNative ? 'border-emerald-200 bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:border-emerald-900 dark:text-emerald-200' : 'border-amber-200 bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:border-amber-900 dark:text-amber-200')}>
        ...
        <div>
          {isNative ? <><b>How to add:</b> long-press your Android home screen → <b>Widgets</b> → find <b>CaCa</b> → drag <i>Today's Classes</i>, <i>Daily Note</i> or <i>Next Alarm</i> onto the screen. They update automatically whenever you change anything in the app.</>
      ...
      <div className="rounded-3xl bg-[radial-gradient(ellipse_at_top,_#1e293b,_#0f172a)] p-6 md:p-10">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3 items-start">
          {/* Today widget 4x3 */}
          <Widget title="Today's Classes" icon={CalendarClock} size="4 × 3" onPin={() => pin('today')}>
            {!snap ? <Skeleton className="h-32 w-full bg-white/10" /> : (
              <>
                <div className="flex items-baseline justify-between mb-2"><span className="text-white font-bold">{format(new Date(), 'EEE, d MMM')}</span><span className="text-[11px] text-white/60">{todayEvents.length} events</span></div>
                {todayEvents.length === 0 ? <div className="py-6 text-center text-white/60 text-sm">Free day 🎉</div> : (
                  <div className="space-y-1.5">{todayEvents.slice(0, 4).map((e, i) => (
                    ...
                    {todayEvents.length > 4 && <div className="text-[11px] text-white/50 text-center">+{todayEvents.length - 4} more</div>}
                  </div>
                )}
              </>
            )}
          ...
          {/* Note widget 3x2 */}
          <Widget title="Daily Note" icon={StickyNote} size="3 × 2" tint="from-amber-400/30 to-amber-600/20" onPin={() => pin('note')}>
            {!snap ? <Skeleton className="h-24 w-full bg-white/10" /> : todayNote ? <p className="text-sm text-white/90 whitespace-pre-line line-clamp-5 leading-snug">{todayNote}</p> : <div className="py-4 text-center text-white/60 text-sm">Tap to write today's note</div>}
          </Widget>
          {/* Alarm widget 2x2 */}
          <Widget title="Next Alarm" icon={AlarmClock} size="2 × 2" tint="from-brand-400/30 to-brand-700/20" onPin={() => pin('alarm')}>
            {!snap ? <Skeleton className="h-16 w-full bg-white/10" /> : nextAlarm ? (
              <div className="text-center py-2"><div className="text-3xl font-bold text-white">{nextAlarm.timeLabel}</div><div className="text-xs text-white/70 mt-1 truncate">{nextAlarm.label}</div></div>
      ...
      <p className="mt-4 text-xs text-slate-400 flex items-center gap-1"><LayoutGrid className="h-3.5 w-3.5" /> Widgets are implemented natively (AppWidgetProvider) in <code>android/app/src/main/java/app/caca/widgets</code>.</p>
    </div>
  );
}

function Widget({ title, icon: Icon, size, tint = 'from-white/10 to-white/5', children, onPin }: { title: string; icon: typeof AlarmClock; size: string; tint?: string; children: React.ReactNode; onPin: () => void }) {
  return (
    <div>
      ...
      <button onClick={onPin} className="mt-2 w-full btn bg-white/10 text-white text-xs py-2 hover:bg-white/20"><Plus className="h-3.5 w-3.5" /> Add to home screen</button>
    </div>
  );
}
